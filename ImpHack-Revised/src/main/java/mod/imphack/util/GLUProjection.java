...
public final class GLUProjection {
	...
	/**
	 * Updates the matrices. Needed whenever the viewport or one of the matrices has
	 * changed.
	 *
	 ...
	 */
	public void updateMatrices(IntBuffer viewport, FloatBuffer modelview, FloatBuffer projection, double widthScale,
			double heightScale) {
		...
		this.fovX = (float) Math.toDegrees(2.0D
				* Math.atan((this.displayWidth / this.displayHeight) * Math.tan(Math.toRadians(this.fovY) / 2.0D)));
		...
		this.frustum = this.getFrustum(this.frustumPos.x, this.frustumPos.y, this.frustumPos.z, yaw, pitch, fov, 1.0F,
				displayWidth / displayHeight);
		this.invFrustum = this.getFrustum(this.frustumPos.x, this.frustumPos.y, this.frustumPos.z, yaw - 180, -pitch,
				fov, 1.0F, displayWidth / displayHeight);
		// Set view vec
		this.viewVec = this.getRotationVector(yaw, pitch).normalized();
		// Calculate screen border angles
		this.bra = Math.toDegrees(Math
				.acos((displayHeight * heightScale) / Math.sqrt(displayWidth * widthScale * displayWidth * widthScale
						+ displayHeight * heightScale * displayHeight * heightScale)));
	...
	/**
	 * Uses
	 * {@link GLU#gluProject(float, float, float, FloatBuffer, FloatBuffer, IntBuffer, FloatBuffer)}
	 * to project world space coordinates to screen space coordinates.
	 *
	 * @param x                X position
	 * @param y                Y position
	 * @param z                Z position
	 * @param clampModeOutside Clamp mode used when the point is outside the normal
	 *                         and inverted frustum
	 * @param extrudeInverted  If set to true this extrudes the projected point onto
	 *                         the screen borders if the point is inside the
	 *                         inverted frustum
	 */
		...
		if (this.viewport != null && this.modelview != null && this.projection != null) {
			...
			if (outsideFrustum) {
				...
				if ((extrudeInverted && !outsideInvertedFrustum)
						|| (outsideInvertedFrustum && clampModeOutside != ClampMode.NONE)) {
					if ((extrudeInverted && !outsideInvertedFrustum)
							|| (clampModeOutside == ClampMode.DIRECT && outsideInvertedFrustum)) {
						// Point in inverted frustum, has to be clamped
						double vecX;
						double vecY;
						if (GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection,
								this.viewport, this.coords)) {
							// Get projected coordinates
							if (opposite) {
								// Invert coordinates
								vecX = this.displayWidth * this.widthScale
										- (double) this.coords.get(0) * this.widthScale
										- this.displayWidth * this.widthScale / 2.0F;
								vecY = this.displayHeight * this.heightScale
										- (displayHeight - (double) this.coords.get(1))
												* this.heightScale
										- this.displayHeight * this.heightScale / 2.0F;
							} else {
								vecX = (double) this.coords.get(0) * this.widthScale
										- this.displayWidth * this.widthScale / 2.0F;
								vecY = (this.displayHeight - (double) this.coords.get(1))
										* this.heightScale - this.displayHeight * this.heightScale / 2.0F;
							}
						} else {
						...
						vecY = vec.y;
						// Get vector line
						Line vectorLine = new Line(this.displayWidth * this.widthScale / 2.0F,
								this.displayHeight * this.heightScale / 2.0F, 0, vecX, vecY, 0);
						...
						return new Projection(intersect.x, intersect.y,
								outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED);
					} else if ((clampModeOutside == ClampMode.ORTHOGONAL && outsideInvertedFrustum)) {
						if (GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection,
								this.viewport, this.coords)) {
							// Get projected coordinates
							double guiX = (double) this.coords.get(0) * this.widthScale;
							double guiY = (this.displayHeight - (double) this.coords.get(1))
									* this.heightScale;
							if (opposite) {
								// Invert coordinates
							...
							return new Projection(guiX, guiY,
									outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED);
						} else {
							return new Projection(0, 0, Projection.Type.FAIL);
						}
	...
	/**
	 * Returns the frustum corner points 0 -------- 3 | | | | 1 -------- 2
	 *
	 * @param x             X position
	 * @param y             Y position
	 * @param z             Z position
	 ...
	 */
	public Vector3D[] getFrustum(double x, double y, double z, double rotationYaw, double rotationPitch, double fov,
			double farDistance, double aspectRatio) {
		...
		Vector3D fc = new Vector3D(view_camPos_product.x * farDistance, view_camPos_product.y * farDistance,
				view_camPos_product.z * farDistance);
		Vector3D topLeftfrustum = new Vector3D(fc.x + (up.x * hFar / 2D) - (right.x * wFar / 2D),
				fc.y + (up.y * hFar / 2D) - (right.y * wFar / 2D), fc.z + (up.z * hFar / 2D) - (right.z * wFar / 2D));
		Vector3D downLeftfrustum = new Vector3D(fc.x - (up.x * hFar / 2D) - (right.x * wFar / 2D),
				fc.y - (up.y * hFar / 2D) - (right.y * wFar / 2D), fc.z - (up.z * hFar / 2D) - (right.z * wFar / 2D));
		Vector3D topRightfrustum = new Vector3D(fc.x + (up.x * hFar / 2D) + (right.x * wFar / 2D),
				fc.y + (up.y * hFar / 2D) + (right.y * wFar / 2D), fc.z + (up.z * hFar / 2D) + (right.z * wFar / 2D));
		Vector3D downRightfrustum = new Vector3D(fc.x - (up.x * hFar / 2D) + (right.x * wFar / 2D),
				fc.y - (up.y * hFar / 2D) + (right.y * wFar / 2D), fc.z - (up.z * hFar / 2D) + (right.z * wFar / 2D));
		return new Vector3D[] { topLeftfrustum, downLeftfrustum, downRightfrustum, topRightfrustum };
	}

	/**
	 * Returns the frustum that has been constructed with
	 * {@link GLUProjection#updateMatrices(IntBuffer, FloatBuffer, FloatBuffer, double, double)}
	 * 0 -------- 3 | | | | 1 -------- 2
	 *
	 */
